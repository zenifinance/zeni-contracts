...
import "../interface/IZeniCallee.sol";

// Zeni Fees contract is used as a 1:1 pair relationship to split out fees, this ensures that the curve does not need to be modified for LP shares
contract ZeniFees {
...
// The Zeni pair of pools, either stable or volatile
contract ZeniPair {
    ...
    uint private constant PERIOD_SIZE = 1800;
    ...
    constructor() {
        factory = msg.sender;
        (address _token0, address _token1, bool _stable) = ZeniFactory(msg.sender).getInitializable();
        (token0, token1, stable) = (_token0, _token1, _stable);
        fees = address(new ZeniFees(_token0, _token1));
        if (_stable) {
    ...
    function claimFeesFor(address recipient) public lock returns (uint claimed0, uint claimed1) {
        ...
        ZeniFees(fees).claimFeesFor(recipient, claimed0, claimed1);
    ...
    function _update0(uint amount) internal {
        _safeTransfer(token0, fees, amount); // transfer the fees out to ZeniFees
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, "ILM"); // Zeni: INSUFFICIENT_LIQUIDITY_MINTED
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, "ILB"); // Zeni: INSUFFICIENT_LIQUIDITY_BURNED
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(!ZeniFactory(factory).isPaused());
        require(amount0Out > 0 || amount1Out > 0, "IOA"); // Zeni: INSUFFICIENT_OUTPUT_AMOUNT
        (uint _reserve0, uint _reserve1) =  (reserve0, reserve1);
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "IL"); // Zeni: INSUFFICIENT_LIQUIDITY

        uint _balance0;
        uint _balance1;
        { // scope for _token{0,1}, avoids stack too deep errors
        (address _token0, address _token1) = (token0, token1);
        require(to != _token0 && to != _token1, "IT"); // Zeni: INVALID_TO
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) IZeniCallee(to).hook(msg.sender, amount0Out, amount1Out, data); // callback, used for flash loans
        _balance0 = IERC20(_token0).balanceOf(address(this));
        _balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = _balance0 > _reserve0 - amount0Out ? _balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = _balance1 > _reserve1 - amount1Out ? _balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, "IIA"); // Zeni: INSUFFICIENT_INPUT_AMOUNT
        ...
        require(_k(_balance0, _balance1) >= _k(_reserve0, _reserve1), "K"); // Zeni: K
        }
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, "Zeni: EXPIRED");
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, "Zeni: INVALID_SIGNATURE");
...
contract ZeniFactory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(ZeniPair).creationCode);
    }
    ...
    function createPair(address tokenA, address tokenB, bool stable) external returns (address pair) {
        require(tokenA != tokenB, "IA"); // Zeni: IDENTICAL_ADDRESSES
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "ZA"); // Zeni: ZERO_ADDRESS
        require(getPair[token0][token1][stable] == address(0), "PE"); // Zeni: PAIR_EXISTS - single check is sufficient
        bytes32 salt = keccak256(abi.encodePacked(token0, token1, stable)); // notice salt includes stable as well, 3 parameters
        (_temp0, _temp1, _temp) = (token0, token1, stable);
        pair = address(new ZeniPair{salt:salt}());
